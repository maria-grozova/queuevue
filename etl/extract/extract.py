...
def save_parks_metadata():
    ...
    try:
        ...
        try:
            ...
            print(f"Error decoding JSON for park_id {park_id}: {e}")
            data = None
...
# Get raw queue times for one park and return JSON with timestamp
...
# Define the number of iterations for fetching queue times
FETCH_ITERATIONS = 12
...
def update_queue_times_every_5_minutes():
    ...
    park_ids = [park["id"] for park in parks_data]

    for i in range(FETCH_ITERATIONS):  # Run FETCH_ITERATIONS times (every 5 min for 30 minutes)
        print(f"Fetching queue times iteration {i+1}/12...")
